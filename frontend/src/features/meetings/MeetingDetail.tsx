  ...
  const ask = useMutation({
    ...
    onSuccess: async (reply) => {
      ...
      */
      ...
      /*
        ★ 초안을 만들면 **회의록 칸으로 데려간다** (2026-09-08).

        ── 무엇이 문제였나 ──────────────────────────────────
        전에는 *"아래에서 고친 뒤 저장하세요"* 라고만 했다. 그런데
        **「아래」가 틀렸다** — 초안 단추는 화면 맨 아래에 있고 회의록 칸은
        그 **위**에 있다. 게다가 「전사문」 탭을 보고 있으면 회의록 칸이
        아예 안 보인다.

        부장님 — *"아래 어디에서 고치라는 거니"*

        말을 고치는 것으로는 부족하다. **화면을 그 자리로 옮겨 준다.**
        어디를 보라고 말하는 것보다 그 자리를 띄워 주는 편이 낫다.
      */
      setTab('회의록')
      setNote(
        hasContent(reply.doc)
          ? '초안이 위 「회의록」 칸에 채워졌습니다. 고치신 뒤 맨 아래 「저장」을 눌러 주세요.'
          : '⚠️ AI 가 답을 줬지만 양식으로 나누지 못했습니다. 위 「회의록」 칸의 「AI 원본」을 확인해 주세요.',
      )
      void qc.invalidateQueries({ queryKey: ['meetings'] })
    },
