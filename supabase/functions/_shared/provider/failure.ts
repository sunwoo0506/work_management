 ...
 *
 * 429 는 서로 다른 **네 가지**를 한 숫자로 말한다 —
 *   ① 잠깐 몰렸다          → 기다리면 풀린다
 *   ② 분당 한도를 넘었다    → 기다리면 풀린다 (잔액과 무관)
 *   ③ **지출 상한에 걸렸다** → **기다려도 안 풀린다.** 사람이 상한을 올려야 한다
 *   ④ 쓸 돈이 떨어졌다      → 크레딧을 채워야 한다 (한도와 **잔액은 다르다**)
 *
 * ③ 을 못 갈라서 「곧 다시 시도합니다」라고 띄운 적이 있다 (2026-09-08).
 * 거짓말인 데다 시간만 버렸다.
 *
...
export function explainFailure(status: number, raw: string, provider = 'openai'): Failure {
  ...
  */
  /*
    ★ **지출 상한에 걸린 것은 기다려도 안 풀린다** (2026-09-08).

    이것도 429 로 온다. 그런데 아래 「몰렸다」와 성격이 정반대다 —
    시간이 지나서 풀리는 게 아니라 **사람이 상한을 올려야** 풀린다.

    그런데 우리는 「요청이 몰려 잠시 거절됐습니다. 곧 다시 시도합니다」라고
    띄우고 실제로 두 번 더 두드렸다. **거짓말인 데다 시간만 버린다.**
    사용자는 기다리다가 「왜 안 되지」로 끝난다.

    ⚠️ 이건 이 파일이 생긴 이유와 **똑같은 실수**다. 429 하나에 서로 다른
       사정이 여럿 들어 있는데, 그중 하나를 못 갈라서 엉뚱한 안내를 했다.
       2026-08-16 · 2026-09-05 에 이어 세 번째다.
  */
  if (/spend cap|spending cap|spend_cap|ai\.studio\/spend/i.test(message)) {
    return {
      error:
        `**${vendor(provider)} 쪽 이번 달 지출 상한에 걸렸습니다.** 기다려도 안 풀립니다 — ` +
        '상한을 올리거나 다음 달까지 기다려야 합니다. ' +
        (provider.toLowerCase().startsWith('gemini')
          ? 'Google AI Studio 의 지출 상한 화면(https://ai.studio/spend)에서 올릴 수 있습니다.'
          : '공급자 결제 화면에서 상한을 확인해 주세요.') +
        ' 변환하지 못한 소리는 회의록에 보관되니 **상한을 올린 뒤 「다시 변환」**하시면 됩니다.' +
        said,
      retryable: false,
    }
  }
